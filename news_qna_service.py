...
class NewsQnAService:
    ...
    def generate(self, question: str, docs: List[Dict[str, Any]]) -> str:
      ...
      답변을 작성 시 아래 지침을 반드시 지켜주세요.
        ...
        [답변예시]
        📊 현황 요약
        최근 엔비디아 주가가 30일 종가 기준 100일 이동평균선 아래로 하락하면서 기술적 지표상 부정적인 신호가 발생했습니다. 
        특히 변동성이 비트코인보다도 2배 이상 높아졌다는 점이 투자자 불안 심리를 키우고 있습니다.
        ...
        ✦ **따라서 리스크 관리와 분할 매수 전략**이 균형 잡힌 접근법이 될 것입니다.
