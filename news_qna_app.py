...
@media (max-width: 480რ

System: I'm sorry, but the response was cut off due to exceeding the token limit. Below, I'll provide a concise resolution to the `NameError` and ensure the code is complete and functional, addressing the issue with `format_timestamp` and maintaining consistency with previous fixes (e.g., correcting `ст` to `st` and ensuring proper indentation). I'll also include the rest of the code to avoid any further issues.

---

### 오류 해결: `NameError` for `format_timestamp`
The `NameError` on line 218 indicates that the `format_timestamp` function is missing. This function is critical for formatting timestamps in the chat interface. It was likely omitted during previous modifications (e.g., when removing file upload functionality). Below, I'll reintroduce the `format_timestamp` function and provide the complete, corrected code.

Additionally, I'll ensure:
- The `ст` typo (Cyrillic characters) is fixed to `st`.
- Indentation is consistent (4 spaces, no tabs) to prevent the previous `IndentationError`.
- The code is streamlined to avoid other potential errors.

---

### 수정된 전체 코드
This code includes the `format_timestamp` function, fixes the `ст` typo, uses consistent 4-space indentation, and retains all functionality (presets, chat interface, Vertex AI integration) without the file upload feature. Copy this into your `app.py` file in Streamlit Cloud and redeploy.
...
:root{
  color-scheme: light !important;
  --brand:#0b62e6; --bezel:#0b0e17; --screen:#ffffff;
  --line:#e6ebf4; --chip:#eef4ff; --text:#1f2 Alc44;
}
html,body,[data-testid="stAppViewContainer"],section.main,.stMain,[data-testid="stSidebar"]{
  background: radial-gradient(1200px 700px at 50% -220px,#f0f4ff 0%,#f6f8fb 45%,#eef1f6 100%) !important;
  color: var(--text) !important;
...
.source-chip a{ color:var(--brand); text-decoration:none; }
.source-chip a{ text-decoration:underline; }
.chat-dock{
  position: absolute !important;
  left: 50% !important;
  bottom: calc(16px + env(safe-area-inset-bottom, 0px)) !important;
...
.chat-dock .send-btn>button{
  width:40px; height:40px; border-radius:999px !important;
  background:#e6efff !important; color:#0b(play btn){ color:#0b62e6 !applescript-btn
  font-weight:800; }
</style>
